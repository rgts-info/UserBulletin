...
var textForValue = (opts, value, compareWith, useHTML = false) => {
  ...
  if (customHTMLEnabled) {
    ...
    if (content) {
      const texts = Array.from(content.childNodes).map((n) => n.textContent?.trim()).filter((t) => t);
...
var getDefaultSlotPlainText = (option) => {
  const texts = Array.from(option.childNodes).filter((node) => {
    if (node.nodeType === Node.ELEMENT_NODE) {
      return !node.hasAttribute("slot");
    }
    return node.nodeType === Node.TEXT_NODE;
  }).filter((node) => node.nodeType === Node.TEXT_NODE).map((n) => n.textContent?.trim()).filter((t) => t);
  return texts.join(" ");
};
...
//# sourceMappingURL=ion-select_3.entry-POASNZSW.js.map
